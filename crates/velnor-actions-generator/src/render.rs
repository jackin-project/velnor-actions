...
use crate::model::{ClassContract, ForkTable, Gate, Lane};
...
pub fn consumer_template(class: RepositoryClass) -> String {
    consumer_template_with_forks(class, &ForkTable::canonical())
}

/// Render a class consumer from the checked-in fork table.
#[must_use]
pub fn consumer_template_with_forks(class: RepositoryClass, forks: &ForkTable) -> String {
    ...
    for fork in forks.entries() {
        let owner = fork.owner.as_str();
        let env_prefix = owner_env_prefix(owner);
        b.line(
            10,
            &format!("{env_prefix}_RESULT: ${{{{ needs.{owner}.result }}}}"),
        );
        b.line(
            10,
            &format!("{env_prefix}_CONTRACT: ${{{{ needs.{owner}.outputs.contract }}}}"),
        );
    }
    b.line(8, "shell: bash");
...
pub fn callable_workflow(
    ...
    callable_workflow_with_forks(contract, caches, block_sha, &ForkTable::canonical())
}

/// Render a callable workflow whose caller verification is derived from the
/// same fork table as its consumer template.
#[must_use]
pub fn callable_workflow_with_forks(
    contract: &ClassContract,
    caches: &CacheContract,
    block_sha: &str,
    forks: &ForkTable,
    ...
    b.line(
        ...
        &format!(
            ...
            hex::encode(Sha256::digest(
                consumer_template_with_forks(class, forks).as_bytes(),
            ))
        ),
    );
...
/// Render caller verification from the checked-in fork table. The canonical
/// constant is retained for script-level tests; generated callables always use
/// this function so owner count, set, and placeholder mapping stay data-driven.
pub fn validate_request_script(forks: &ForkTable) -> String {
    let mut script = VALIDATE_REQUEST_SCRIPT.to_string();
    let owners: Vec<_> = forks
        .entries()
        .iter()
        .map(|fork| fork.owner.as_str())
        .collect();
    let mut sorted_owners = owners.clone();
    sorted_owners.sort_unstable();
    let owner_regex = owners.join("|");
    let owner_set = sorted_owners.join(",");
    let owner_names = owners.join(" ");
    let placeholder_names = forks
        .entries()
        .iter()
        .map(|fork| {
            fork.placeholder
                .trim_matches('@')
                .trim_end_matches("_FLEET_SHA")
        })
        .collect::<Vec<_>>()
        .join("|");
    ...
    script = script.replace("== 3", &format!("== {}", forks.entries().len()));
    script = script.replace(
    ...
    script = script.replace(
        "for owner in jackin-project tailrocks ChainArgos; do",
        &format!("for owner in {owner_names}; do"),
    );
    script = script.replace(
        "@(JACKIN|TAILROCKS|CHAINARGOS)_FLEET_SHA@",
        &format!("@({placeholder_names})_FLEET_SHA@"),
    );
    let old_case = "    case \"${owner}\" in\n      jackin-project) placeholder='@JACKIN_FLEET_SHA@' ;;\n      tailrocks) placeholder='@TAILROCKS_FLEET_SHA@' ;;\n      ChainArgos) placeholder='@CHAINARGOS_FLEET_SHA@' ;;\n    esac";
    let mut new_case = String::from("    case \"${owner}\" in\n");
    for fork in forks.entries() {
        new_case.push_str(&format!(
            "      {}) placeholder='{}' ;;\n",
            fork.owner, fork.placeholder
        ));
    }
    new_case.push_str("    esac");
    script.replace(old_case, &new_case)
}
...
pub fn render_consumer(
    ...
    render_consumer_with_forks(template, release_shas, calver, &ForkTable::canonical())
}

/// Materialize a consumer template using the owner order and placeholder map
/// supplied by the fork table.
pub fn render_consumer_with_forks(
    template: &str,
    release_shas: [&str; 3],
    calver: &str,
    forks: &ForkTable,
) -> Result<String, String> {
    if forks.entries().len() != release_shas.len() {
        return Err(format!(
            "release has {} owner SHAs, fork table has {} entries",
            release_shas.len(),
            forks.entries().len()
        ));
    }
    ...
    for (fork, sha) in forks.entries().iter().zip(release_shas) {
        if out.matches(&fork.placeholder).count() != 1 {
            return Err(format!(
                "expected exactly one {} placeholder",
                fork.placeholder
            ));
        }
        out = out.replace(&fork.placeholder, &format!("@{sha}"));
    }
...
fn owner_env_prefix(owner: &str) -> String {
    owner
        .chars()
        .map(|character| {
...
fn ci_required_script(forks: &ForkTable) -> String {
    if *forks == ForkTable::canonical() {
        return CI_REQUIRED_SCRIPT.to_string();
    }
    let mut script = String::from(
        "set -euo pipefail\n# Positive truth table: exactly one selected owner must succeed; every other owner must skip.\ncase \"${OWNER}\" in\n",
    );
    for (index, selected) in forks.entries().iter().enumerate() {
        let selected_prefix = owner_env_prefix(&selected.owner);
        let others: Vec<_> = forks
            .entries()
            .iter()
            .enumerate()
            .filter(|(other_index, _)| *other_index != index)
            .map(|(_, fork)| owner_env_prefix(&fork.owner))
            .collect();
        script.push_str(&format!("  {})\n", selected.owner));
        script.push_str(&format!(
            "    sel_result=\"${{{}_RESULT}}\"; sel_contract=\"${{{}_CONTRACT}}\"\n",
            selected_prefix, selected_prefix
        ));
        for (slot, prefix) in ["a", "b"].into_iter().zip(others) {
            script.push_str(&format!(
                "    {slot}_result=\"${{{prefix}_RESULT}}\"; {slot}_contract=\"${{{prefix}_CONTRACT}}\"\n"
            ));
        }
        script.push_str("    ;;\n");
    }
    script.push_str(
        "  *) echo \"ci-required: unrecognized owner '${OWNER}'\" >&2; exit 1 ;;\nesac\n",
    );
    script.push_str(
        "if [ \"${sel_result}\" != \"success\" ] || [ \"${sel_contract}\" != \"success\" ]; then echo \"ci-required: selected owner did not succeed\" >&2; exit 1; fi\n",
    );
    script.push_str(
        "if [ \"${a_result}\" != \"skipped\" ] || [ \"${b_result}\" != \"skipped\" ]; then echo \"ci-required: a non-selected owner ran (expected both 'skipped')\" >&2; exit 1; fi\n",
    );
    script.push_str(
        "if [ -n \"${a_contract}\" ] || [ -n \"${b_contract}\" ]; then echo \"ci-required: a non-selected owner produced a contract output (expected empty)\" >&2; exit 1; fi\n",
    );
    script.push_str("echo \"ci-required: fleet contract satisfied for owner '${OWNER}'\"");
    script
}
